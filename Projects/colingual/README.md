...
Only variables prefixed with `VITE_` are exposed to browser code. Keep provider API keys in server-only variables such as `AI_ASSISTANT_API_KEY`, `ELEVENLABS_API_KEY`, and `OPENAI_API_KEY`; use `VITE_AI_ASSISTANT_ENDPOINT` and `VITE_TTS_ENDPOINT` for browser-to-backend proxy URLs.

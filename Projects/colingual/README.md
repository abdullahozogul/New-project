...
# Option A — browser key (dev only; ignored in production builds):
VITE_GEMINI_API_KEY=
...
# Production TTS must use a backend endpoint; browser provider keys are dev-only.
VITE_TTS_ENDPOINT=
...
Only variables prefixed with `VITE_` are exposed to browser code. For production, call Gemini/TTS from a server or edge function and point `VITE_AI_ASSISTANT_ENDPOINT` / `VITE_TTS_ENDPOINT` at it; keep provider API keys out of the client bundle.

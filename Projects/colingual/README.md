...
Only variables prefixed with `VITE_` are exposed to browser code. Provider API keys must stay server-side: use `AI_ASSISTANT_API_KEY`, `ELEVENLABS_API_KEY`, and `OPENAI_API_KEY` in a backend or the local Vite dev proxies, then point the browser at `VITE_AI_ASSISTANT_ENDPOINT` and `VITE_TTS_ENDPOINT` for production.

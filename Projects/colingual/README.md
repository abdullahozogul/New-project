...
VITE_GEMINI_MODEL=
# Public backend POST endpoints; request body includes `model`, `systemInstruction`, `contents`
...
Only variables prefixed with `VITE_` are exposed to browser code. Keep provider API keys in server-only variables such as `AI_ASSISTANT_API_KEY`, `ELEVENLABS_API_KEY`, and `OPENAI_API_KEY`; point the public `VITE_AI_ASSISTANT_ENDPOINT` and `VITE_TTS_ENDPOINT` values at backend or edge functions that forward requests.

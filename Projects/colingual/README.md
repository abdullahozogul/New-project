...
VITE_GEMINI_MODEL=
# Browser-safe backend POST endpoints; request body includes provider payloads, keys stay server-side.
...
Only variables prefixed with `VITE_` are exposed to browser code. Keep provider API keys in server-only variables such as `AI_ASSISTANT_API_KEY`, `ELEVENLABS_API_KEY`, and `OPENAI_API_KEY`; point the public endpoint variables at your server or edge functions.

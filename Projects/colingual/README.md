...
# Chat coach/live CEFR: browser calls your backend proxy.
VITE_GEMINI_MODEL=
# Your backend POST endpoint; request body includes `model`, `systemInstruction`, `contents`.
VITE_AI_ASSISTANT_ENDPOINT=
# Your backend TTS endpoint; keep ElevenLabs/OpenAI keys server-side.
VITE_TTS_ENDPOINT=
...
Only variables prefixed with `VITE_` are exposed to browser code. Provider API keys must use server-only names such as `AI_ASSISTANT_API_KEY`, `ELEVENLABS_API_KEY`, and `OPENAI_API_KEY`; point `VITE_AI_ASSISTANT_ENDPOINT` and `VITE_TTS_ENDPOINT` at backend routes that hold those secrets.

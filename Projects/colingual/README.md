...
VITE_GEMINI_MODEL=
# Backend POST endpoint; request body includes `model`, `systemInstruction`, `contents`
VITE_AI_ASSISTANT_ENDPOINT=
# Backend POST endpoint that returns audio/mpeg for TTS requests.
VITE_TTS_ENDPOINT=
...
Only variables prefixed with `VITE_` are exposed to browser code. Keep provider API keys in server-only variables such as `AI_ASSISTANT_API_KEY`, `ELEVENLABS_API_KEY`, and `OPENAI_API_KEY`; call them from a server or edge function and point `VITE_AI_ASSISTANT_ENDPOINT` / `VITE_TTS_ENDPOINT` at that backend.

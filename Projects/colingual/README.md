...
VITE_GEMINI_MODEL=
# Chat coach/live CEFR: your backend POST endpoint; request body includes `model`, `systemInstruction`, `contents`
VITE_AI_ASSISTANT_ENDPOINT=
# Cloud TTS: your backend POST endpoint returning audio/mpeg for a TTSRequest body
VITE_TTS_ENDPOINT=
...
Only variables prefixed with `VITE_` are exposed to browser code. Keep provider secrets such as Gemini, ElevenLabs, and OpenAI keys in server-only variables, then point `VITE_AI_ASSISTANT_ENDPOINT` and `VITE_TTS_ENDPOINT` at trusted backend proxies.

...
# Production backend POST endpoint for TTS synthesis.
VITE_TTS_ENDPOINT=
...
Only variables prefixed with `VITE_` are exposed to browser code. Keep provider API keys server-side; for local development the Vite middleware reads server-only keys from `.env.local`, and production should point the `VITE_*_ENDPOINT` variables at backend or edge functions.

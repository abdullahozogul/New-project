...
Only variables prefixed with `VITE_` are exposed to browser code. Keep provider API keys in server-only variables (`AI_ASSISTANT_API_KEY`, `ELEVENLABS_API_KEY`, `OPENAI_API_KEY`) and point `VITE_AI_ASSISTANT_ENDPOINT` / `VITE_TTS_ENDPOINT` at backend or edge functions in production.

...
# Chat coach: uses Google Gemini **gemini-2.5-flash** by default via a backend/proxy.
...
Only variables prefixed with `VITE_` are exposed to browser code. Keep provider API keys out of the client bundle; use `VITE_AI_ASSISTANT_ENDPOINT` / `VITE_TTS_ENDPOINT` or the local dev proxy with server-side keys.

...
function ttsEndpoint(): string | null {
  if (import.meta.env.VITE_TTS_ENDPOINT?.trim()) {
    return import.meta.env.VITE_TTS_ENDPOINT.trim()
  }
...
async function fetchSynthesizeFromApi(request: TTSRequest, endpoint: string): Promise<ArrayBuffer> {

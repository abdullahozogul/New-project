...
function configuredTtsEndpoint(): string | null {
  const endpoint = import.meta.env.VITE_TTS_ENDPOINT?.trim()
  if (endpoint) {
    return endpoint
  }
...
async function fetchSynthesize(endpoint: string, request: TTSRequest): Promise<ArrayBuffer> {
...
async function synthesizeBuffer(request: TTSRequest): Promise<{ buffer: ArrayBuffer; provider: TTSProvider }> {
  ...
  return { buffer: await fetchSynthesize(endpoint, request), provider: PROVIDER_RULES[request.useCase] }
}

export function isCloudTtsConfigured(): boolean {
  return Boolean(configuredTtsEndpoint())
}

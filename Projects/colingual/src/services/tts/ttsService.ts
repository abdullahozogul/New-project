...
function ttsProxyEndpoint(): string | null {
  const configuredEndpoint = import.meta.env.VITE_TTS_ENDPOINT?.trim()
...
async function fetchSynthesizeFromProxy(request: TTSRequest): Promise<ArrayBuffer> {
  const endpoint = ttsProxyEndpoint()
  if (!endpoint) {

...
function devTtsProxyEndpoint(): string | null {
...
function resolveTtsEndpoint(): string {
  return import.meta.env.VITE_TTS_ENDPOINT?.trim() || devTtsProxyEndpoint() || ''
}

async function fetchSynthesizeFromEndpoint(endpoint: string, request: TTSRequest): Promise<ArrayBuffer> {
...
async function synthesizeBuffer(request: TTSRequest): Promise<{ buffer: ArrayBuffer; provider: TTSProvider }> {
  ...
  const buffer = await fetchSynthesizeFromEndpoint(endpoint, request)
